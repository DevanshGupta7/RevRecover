...
def get_current_organisation(db: Session, organisation_id: UUID):
    """
    Return the organisation belonging to the current user.
    """

    organisation = get_organisation_by_id(db=db, organisation_id=organisation_id)
...
def update_current_organisation(
    db: Session, organisation_id: UUID, request: OrganisationUpdateRequest
):
    """
    Update the current organisation.
    """

    organisation = get_current_organisation(db=db, organisation_id=organisation_id)
    ...
    return update_organisation(db=db, organisation=organisation)
